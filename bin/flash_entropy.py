"""
Flash search for entropy / reverse entropy (weighted)
...
class FlashEntropy:
    def __init__(self, max_ms2_tolerance_in_da=0.024, mz_index_step=0.0001, path_data=None,
        ...
        self.similarity_search = FlashEntropyCore(path_data=path_data,
                                                  max_ms2_tolerance_in_da=max_ms2_tolerance_in_da,
                                                  mz_index_step=mz_index_step,
                                                  sqrt_transform=sqrt_transform)

    def identity_search(self, precursor_mz, peaks, ms1_tolerance_in_da, ms2_tolerance_in_da, reverse, **kwargs):
        """
    ...
    def open_search(self, peaks, ms2_tolerance_in_da, reverse, **kwargs):
        """
    ...
    def search(self,
        ...
        :param noise_threshold: The intensity threshold for removing the noise peaks. The peaks with intensity smaller than noise_threshold * max(intensity)
                                will be removed. Default is 0.01.
        :param min_ms2_difference_in_da:    The minimum difference between two peaks in the MS/MS spectrum. Default is 0.05.
        :param reverse:  Whether to perform the reverse search. Default is False.
...
class FlashEntropyCore:
    ...
    def search(
        ...
        spec_usage_arr = np.zeros(self.total_spectra_num, dtype=np.float32)
        ...
        if search_type == 0:
            ...
            # transform query spectrum intensity
            ...
            if reverse:
                ...
                tmp_entropy = -np.sum(match_table_q_norm * np.log(match_table_q_norm + 1e-6), axis=0)

                # Find columns where entropy is less than 3 and col sum is not zero
                low_entropy_cols = np.where((tmp_entropy < 3) & (np.sum(match_table_q, axis=0) > 0))[0]
                ...
                match_table_q_corrected = match_table_q_corrected / (np.sum(match_table_q_corrected, axis=0) + 1e-6)
            ...
            if reverse:
                ...
                # calculate similarity
                part_similarity = 0.5 * np.sum(_entropy_helper(match_table_q + match_table_r) -
                                               _entropy_helper(match_table_q) - _entropy_helper(match_table_r), axis=0)
            # count matched peaks
    ...
    def search_hybrid(self, precursor_mz=None, peaks=None, ms2_tolerance_in_da=0.02, reverse=False):
        ...
        :param reverse: Whether to perform the reverse search.
        """
        if not self.index:
            return np.zeros(0, dtype=np.float32)
        if len(peaks) == 0:
        ...
        if reverse:
            ...
            # Find columns where entropy is less than 3 and col sum is not zero
            low_entropy_cols = np.where((tmp_entropy < 3) & (np.sum(match_table_q_all, axis=0) > 0))[0]
            ...
            similarity_arr = 0.5 * np.sum(_entropy_helper(match_table_q_all + match_table_r_all) -
                                          _entropy_helper(match_table_q_all) - _entropy_helper(match_table_r_all),
                                          axis=0)
    ...
    def build_index(self, all_spectra_list: list, max_indexed_mz: float = 1500.00005):
        ...
        return self.total_spectra_num, self.total_peaks_num, self.index
...
def _entropy_helper(table):
    return table * np.log2(table + 1e-10)
...
if __name__ == "__main__":
    ...
    spectral_library = [{
        ...
        "id": "Demo spectrum 3",
        "precursor_mz": 250.0,
        "peaks": np.array([[200.0, 100], [201.0, 5], [202.0, 20], [205.0, 20]], dtype=np.float32),
        "XXX": "YYY",
    }, {
        "precursor_mz": 350.0,
    ...
    ms2_tol = 0.02
    # search
    search_eng = FlashEntropy(max_ms2_tolerance_in_da=ms2_tol * 1.05,
    ...
    search_result = search_eng.search(
        precursor_mz=400.0,
        peaks=[[200.0, 100], [201.0, 20], [202.0, 4.0], [203.0, 3.0], [303.0, 10]],
