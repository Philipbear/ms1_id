...
class FlashRevcosSearchCore:
    ...
    def search(
        ...
        # Start searching
        # Calculate the reverse cosine similarity for this matched peak
        revcos_similarity = np.zeros(self.total_spectra_num, dtype=np.float32)
        revcos_matched_cnt = np.zeros(self.total_spectra_num, dtype=np.uint32)
        # sum(matched peaks intensity) / sum(query peaks intensity)
        revcos_spectral_usage = np.zeros(self.total_spectra_num, dtype=np.float32)
        ...
        if search_type == 0:
            ...
            revcos_spectral_usage = np.sum(match_table_q, axis=0) / np.sum(peaks[:, 1])
            ...
            revcos_similarity = np.sum(match_table_q * match_table_r, axis=0)
            # count matched peaks
            revcos_matched_cnt = np.sum(match_table_q > 0, axis=0)
        elif search_type == 1:
            ...
            # calculate spectral usage
            part_revcos_spectral_usage = np.sum(match_table_q, axis=0) / np.sum(peaks[:, 1])
            # normalize query spectrum intensity in each column
            if self.sqrt_transform:
                match_table_q = np.sqrt(match_table_q)
            match_table_q = match_table_q / np.sqrt(np.sum(np.square(match_table_q), axis=0))
            # calculate reverse cosine similarity
            part_revcos_similarity = np.sum(match_table_q * match_table_r, axis=0)
            # count matched peaks
            part_revcos_matched_cnt = np.sum(match_table_q > 0, axis=0)
            # update the similarity and matched count
            revcos_similarity[search_spectra_idx_min:search_spectra_idx_max] = part_revcos_similarity
            revcos_matched_cnt[search_spectra_idx_min:search_spectra_idx_max] = part_revcos_matched_cnt
            revcos_spectral_usage[search_spectra_idx_min:search_spectra_idx_max] = part_revcos_spectral_usage

        del match_table_q, match_table_r

        return revcos_similarity, revcos_matched_cnt, revcos_spectral_usage
    ...
    def search_hybrid(self, precursor_mz=None, peaks=None, ms2_tolerance_in_da=0.02):
        ...
        # calculate spectral usage
        revcos_spectral_usage = np.sum(match_table_q_max, axis=0) / np.sum(peaks[:, 1])
        # normalize query spectrum intensity in each column
        if self.sqrt_transform:
            match_table_q_all = np.sqrt(match_table_q_all)
        match_table_q_all = match_table_q_all / np.sqrt(np.sum(np.square(match_table_q_all), axis=0))
        # calculate reverse cosine similarity
        revcos_similarity = np.sum(match_table_q_all * match_table_r_all, axis=0)
        # count matched peaks
        revcos_matched_cnt = np.sum(match_table_q_all > 0, axis=0)
...
def _renormalize_intensity(intensity, sqrt_transform):
    if sqrt_transform:
        intensity = np.sqrt(intensity)
    intensity = intensity / np.sqrt(np.sum(np.square(intensity)))
    return intensity


def _revcos_matching(matched_spec_idx, spec_idx, peaks, matched_peak_q_idx, matched_peak_r_idx, library_peaks_intensity,
                     revcos_similarity, revcos_matched_cnt, revcos_spectral_usage, sqrt_transform):
    # get ref matched peak indices in matched_peak_r_idx
    spec_matched_peak_r_idx = np.where(matched_spec_idx == spec_idx)[0]
    # get query matched peak intensities
    spec_matched_peak_q_intensity = peaks[matched_peak_q_idx[spec_matched_peak_r_idx], 1]
    # renormalize query matched peak intensities
    spec_matched_peak_q_intensity_norm = _renormalize_intensity(spec_matched_peak_q_intensity, sqrt_transform)

    revcos_similarity[spec_idx] = np.sum(
        spec_matched_peak_q_intensity_norm * library_peaks_intensity[
            matched_peak_r_idx[spec_matched_peak_r_idx]]
    )
    revcos_matched_cnt[spec_idx] = len(spec_matched_peak_r_idx)
    revcos_spectral_usage[spec_idx] = np.sum(spec_matched_peak_q_intensity) / np.sum(peaks[:, 1])
    return revcos_similarity, revcos_matched_cnt, revcos_spectral_usage


class FlashRevcosSearchCoreLowMemory(FlashRevcosSearchCore):
...
class FlashRevcosSearch:
    def __init__(self, max_ms2_tolerance_in_da=0.024, mz_index_step=0.0001, low_memory=False, path_data=None,
        ...
        self.sqrt_transform = sqrt_transform
        if low_memory:
            self.revcos_search = FlashRevcosSearchCoreLowMemory(path_data=path_data,
                                                                max_ms2_tolerance_in_da=max_ms2_tolerance_in_da,
                                                                mz_index_step=mz_index_step,
                                                                sqrt_transform=sqrt_transform)
        else:
            self.revcos_search = FlashRevcosSearchCore(path_data=path_data,
                                                       max_ms2_tolerance_in_da=max_ms2_tolerance_in_da,
                                                       mz_index_step=mz_index_step,
                                                       sqrt_transform=sqrt_transform)
    ...
    def identity_search(self, precursor_mz, peaks, ms1_tolerance_in_da, ms2_tolerance_in_da, **kwargs):
        ...
        if spectra_idx_min >= spectra_idx_max:
            return np.zeros(self.revcos_search.total_spectra_num, dtype=np.float32), np.zeros(
                self.revcos_search.total_spectra_num, dtype=np.uint32), np.zeros(self.revcos_search.total_spectra_num,
                                                                                 dtype=np.float32)
        else:
            return self.revcos_search.search(method="open", peaks=peaks,
    ...
    def neutral_loss_search(self, precursor_mz, peaks, ms2_tolerance_in_da, **kwargs):
        ...
        :return:    The similarity score for each spectrum in the library, a numpy array with shape (N,), N is the number of spectra in the library.
        """
        return self.revcos_search.search(method="neutral_loss", precursor_mz=precursor_mz, peaks=peaks,
                                         ms2_tolerance_in_da=ms2_tolerance_in_da, search_type=0)
    ...
    def hybrid_search(self, precursor_mz, peaks, ms2_tolerance_in_da, **kwargs):
        ...
        :return:    The similarity score for each spectrum in the library, a numpy array with shape (N,), N is the number of spectra in the library.
        """
        return self.revcos_search.search_hybrid(precursor_mz=precursor_mz, peaks=peaks,
                                                ms2_tolerance_in_da=ms2_tolerance_in_da)
    ...
    def search_mass(self,
        ...
        """
        return self.revcos_search.search_mass(query_mass=mass, ms2_tolerance_in_da=ms2_tolerance_in_da, method=method)
    ...
    def build_index(self,
        ...
        self.revcos_search.build_index(all_spectra_list, max_indexed_mz)
    ...
    def write(self, path_data=None):
        ...
        """
        if path_data is None:
            path_data = self.revcos_search.path_data
        else:
            path_data = Path(path_data)
        ...
        self.revcos_search.write(path_data)
    ...
    def read(self, path_data=None):
        ...
        return self.revcos_search.read(path_data)
    ...
    def save_memory_for_multiprocessing(self):
        ...
        :return:    None
        """
        self.revcos_search.save_memory_for_multiprocessing()
